...
impl StageProtocol {
    ...
    pub(crate) fn json_schema(self) -> &'static str {
        match self {
            ...
            Self::Verify => {
                r#"{"type":"object","properties":{"opsx_status":{"type":"string","enum":["VERIFIED","RETRY","BLOCKED"]},"summary":{"type":"string","description":"Concise stage result. For RETRY, include concrete agenda, change-spec, implementation, or test mistakes, their governing supplied requirements, and the synchronized repair needed. For BLOCKED, identify the external decision or unavailable input required by authoritative requirements themselves."}},"required":["opsx_status","summary"],"additionalProperties":false}"#
            }
            Self::Frontier => {
