...
impl<U: Ui> App<U> {
    ...
    fn run_apply(
        ...
        let base = if state.bootstrap {
            ...
            format!(
                "{change}\n\nImplement or continue implementing this OpenSpec change completely. Preserve correct partial work and run appropriate project checks. Do not archive the change. Treat the assigned slice as the unit of delivery; organise implementation and verification into tasks within it. Worker is a workflow role, not an assumption of limited model capability. Multiple files, subsystems, test cases, or independently testable intermediate steps are not by themselves reasons to replan. Make a focused implementation and verification attempt within the approved scope. Report TOO_LARGE only when that work demonstrates a concrete capacity or scope constraint that ordered tasks within this change cannot resolve. State what you attempted, the observed constraint, why continuing within this change is not viable, and the minimum necessary decomposition. Do not use TOO_LARGE for ordinary difficulty or correctable engineering failures."
            )
        };
